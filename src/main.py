...
def train_text_classification(args):
    ...
    训练临床筛选标准分类模型
    """
    print(f"开始训练临床筛选标准分类模型... 使用模型: {args.model_name}")
    ...
    if 'Multiple' not in category_map:
        print("Warning: 'Multiple' category missing! Accuracy will be low.")
...
def parse_args():
    ...
    # 这里也要改默认模型为 RoBERTa
    tc_train_parser.add_argument("--model_name", type=str, default="hfl/chinese-roberta-wwm-ext", help="预训练模型名称")
    tc_train_parser.add_argument("--batch_size", type=int, default=32, help="批次大小")
    tc_train_parser.add_argument("--epochs", type=int, default=8, help="训练轮数")
    ...
    # TC Predict (分类预测)
    tc_pred_parser = subparsers.add_parser("tc_predict", help="预测分类结果")
